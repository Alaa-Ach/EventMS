            ...
            <tbody id='myTable'>
               ...
               @foreach ($allPart as $Part)
                    ...
                    <tr>
                        ...
                        <td> {{ count($Part->AllComp)}}</td>
                        {{-- <td class='no-export' > <a class="btn btn-danger" href='/Queries/DeleteP/{{$Part->id}}' >Supprimer</a></td> --}}
                        <td class='no-export' >
                            @if (Auth::user()->isAdmin)
                             <a class="btn btn-danger" href='{{ route('Queries.DeleteP_WC', $Part->id) }}' >Supprimer</a>
                             <a class="btn btn-success" href='{{ route('EditForm_WC', $Part->id) }}' >Modifier</a>
                             @endif
                             <a class="btn btn-primary"  {{$Part->Fonction ? '' : "disabled style=pointer-events:none"}} href='{{ route('Queries.Badge_WC', $Part->id) }}' >Badge</a>
                             <a class="btn btn-secondary"   href='{{ route('Facture_WC', $Part->id) }}' >Facture</a>
                        </td>


                    </tr>
                    {{-- COmp --}}
                        @foreach ( $Part->AllComp as $Comp )
                            <tr>
                                <td>{{ $i++ }}</td>

                                <td> {{ $Comp->N_Passport }}</td>
                                <td> {{ $Comp->Nom }}</td>
                                <td> {{ $Comp->Prenom }}</td>
                                <td> {{ $Comp->Fonction ? $Part->Fonction->label : ''}}</td>
                                <td> {{ $Comp->Pays }}</td>
                                <td> {{ $Comp->Transport ? 'Oui' : 'Non' }}</td>
                                <td> {{ count($Part->AllComp)}}</td>
                                {{-- <td class='no-export' > <a class="btn btn-danger" href='/Queries/DeleteP/{{$Part->id}}' >Supprimer</a></td> --}}
                                @if (Auth::user()->isAdmin)
                                <td class='no-export' >
                                    ...
                                    {{-- <a class="btn btn-primary"  {{$Part->Fonction ? '' : "disabled style=pointer-events:none"}} href='{{ route('Queries.BadgeParticipant', $Part->id) }}' >Badge</a> --}}
                                    {{-- <a class="btn btn-secondary"   href='{{ route('Facture', $Part->id) }}' >Facture</a> --}}
                                </td>
                                @endif
                            </tr>
