        ...
        <table class="table table-striped table-hover table-bordered  tablesorter-dark ">
            ...
            <thead class="table-dark ">
                <tr class=''>
                    ...
                    <th scope="col">Accompagnateur De</th>
            ...
            <tbody id='myTable'>
               ...
               @foreach ($allComp as $Comp)
                    <tr>
                        ...
                        <td> {{ $Comp->Nom }}</td>
                        <td> {{ $Comp->Prenom }}</td>
                        <td> {{ $Comp->Fonction ? $Comp->Fonction->label : '' }}</td>
                        <td> {{ $Comp->Pays }}</td>
                        <td> {{ $Comp->Transport ? 'Oui' : 'Non' }}</td>
                        <td> {{ $Comp->withComp->Nom .' '.$Comp->withComp->Prenom }}</td>
                        {{-- <td class='no-export'> Test</td> --}}
                        @if (Auth::user()->isAdmin)
                        <td class='no-export' >
                           <a class="btn btn-primary"  {{$Comp->Fonction && $Comp->Pays ?  '' : "disabled style=pointer-events:none"}}  href='{{ route('Queries.BadgeComp', $Comp->id) }}' >Badge</a>
                           </td>
                        @endif
                    </tr>
                @endforeach
...
<script>
    ...
    $('#Pdf').click(function() {
        ...
        doc.setLineWidth(2);
        doc.text(280, 30, `Les Accompagnateur ${DateNow()}`);
        doc.autoTable({
        ...
        doc.save(`Accompagnateur ${DateNow()}.pdf`);
        // location.reload();
    ...
    $("#Export").on("click", function() {
        ...
        $("table").table2excel({
            ...
            filename: `Accompagnateur ${DateNow()}.xls`, // do include extension
